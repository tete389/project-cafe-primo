...
export default function ProductTableType({ props }) {
  ...
  return props?.map(
    ...
    row.prodStatus === statusSelect &&
      row.typeName === supSelect && (
        <StyledTableRow key={row.prodId}>
          <StyledTableCell sx={{ maxWidth: 100 }} align="left">{row.prodId}</StyledTableCell>
          <StyledTableCell component="th" scope="row" sx={{ maxWidth: 100 }}>
            {row.prodName + " " + "(" + row.typeName + ")"}
          </StyledTableCell>
          <StyledTableCell sx={{ maxWidth: 50 }} align="center">{row.prodPrice}</StyledTableCell>
          <StyledTableCell sx={{ maxWidth: 100 }} align="center">{row.prodImg}</StyledTableCell>
